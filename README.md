...
были реально проверены. Финальный Tavily-поиск новых кандидатов и OpenAI-офферы
не запускались. Google Sheets API не используется, таблица читается без service
account и только на чтение.
...
| Поиск новых блогеров | Полный Tavily → Apify → scoring → OpenAI drafts контур и dry-run реализованы | `python -m src.main --find-real-bloggers --dry-run-final`; финальный платный поиск не запускался |
...
| Офферы | Mock работает; OpenAI Structured Output для реальных финалистов реализован, но не запускался | Перед использованием обязательна ручная проверка |
...
Максимальный план одного запуска: 9 запросов и 45 результатов Tavily, до 20
...
Последовательность: IdealBloggerProfile → Tavily → нормализация, dedup и
исключение эталонов/магазинов → Apify только для Instagram → confidence-aware
scoring → `FINAL_MIN_SCORE` → `MAX_FINAL_CANDIDATES` → OpenAI Structured Output
drafts. YouTube и Telegram остаются на публичном evidence Tavily; отсутствующие
followers и ER сохраняются как `null`.

OpenAI вызывается только после scoring и только для финалистов. Текст предлагает
обсудить возможный бартер, не обещает конкретный товар или коллекцию и не
отправляется автоматически. Каждый результат получает
...
или brand safety. Реальный анализ эталонного портрета через OpenAI выполнен и
зафиксирован в audit; финальная генерация офферов и Tavily-поиск не запускались.
